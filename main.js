...
scene("failed", (causeOfDeath) => {
    const bg = add([
        sprite(LEVELS[currentLevel].backgroundSprite),
        z(-1),
        anchor("center"),
        fixed(),
        pos(width()/2, height()/2),
        scale(LEVELS[currentLevel].backgroundScale)
    ])
...
scene("game", (LEVEL) => {
    ...
    async function reverseGravity() {
        ...
        await wait(5)  // waits 10 seconds before it starts the loop so hindi siya ma run at launch
        await loop(5, async function() {
    ...
    async function reGravity(startAngle, newAngle) {
        tween(
            startAngle,
            newAngle,
            1,
